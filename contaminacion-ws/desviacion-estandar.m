...
Cataluna_stats = std(double(Cataluna_NO2_vals));
Andalucia_stats = std(double(Andalucia_NO2_vals));
Madrid_stats = std(double(Madrid_NO2_vals));

% Mostrar estadísticas
disp('Cataluña: Desviación Estándar');
disp(Cataluna_stats);
disp('Andalucía: Desviación Estándar');
disp(Andalucia_stats);
disp('Madrid: Desviación Estándar');
disp(Madrid_stats);
