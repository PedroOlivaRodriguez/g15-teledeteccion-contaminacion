...
stats = @(x) [mean(x), median(x), std(double(x)), min(x), max(x)];
Cataluna_stats = stats(Cataluna_NO2_vals);
Andalucia_stats = stats(Andalucia_NO2_vals);
Madrid_stats = stats(Madrid_NO2_vals);

% Mostrar estadísticas
disp('Cataluña: Media, Mediana, Desviación, Mínimo, Máximo');
disp(Cataluna_stats);
disp('Andalucía: Media, Mediana, Desviación, Mínimo, Máximo');
disp(Andalucia_stats);
disp('Madrid: Media, Mediana, Desviación, Mínimo, Máximo');
disp(Madrid_stats);
